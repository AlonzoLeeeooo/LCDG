...
parser.add_argument('--outdir', type=str, help='path to write the output score value')
...
def compute_statistics_of_path(path, model, batch_size, dims, device, num_workers=8):
    if path.endswith('.npz'):
        ...
        files = get_files_from_txt(path)
        m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device, num_workers)
    else:
        path = pathlib.Path(path)
        files = sorted(glob.glob(os.path.join(path, "*.png"))) + sorted(glob.glob(os.path.join(path, "*.jpg")))
        m, s = calculate_activation_statistics(files, model, batch_size,
                                               dims, device, num_workers)
