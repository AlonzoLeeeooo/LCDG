...
class HEDdetector:
    def __init__(self):
        ...
        modelpath = os.path.join('hed-checkpoints', "network-bsds500.pth") # os.path.join(annotator_ckpts_path, "network-bsds500.pth")
        if not os.path.exists(modelpath):
            from basicsr.utils.download_util import load_file_from_url
            load_file_from_url(remote_model_path, model_dir='./hed-checkpoints')
...
if __name__ == "__main__":
    ...
    for path in input_list:
        ...
        hed = edge_detector(image)
        cv2.imwrite(os.path.join(args.outdir, filename + '.png'), hed)